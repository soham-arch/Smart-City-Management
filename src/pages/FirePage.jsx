...
import { requestFireDispatch } from '../lib/cppDispatchApi';
import { computeFireResources } from '../lib/resourceCalculators';
...
const FirePage = () => {
  ...
  const handleDispatch = async () => {
    ...
    try {
      const apiResult = await requestFireDispatch({
        origin: selectedOrigin,
        intensity,
        buildingType,
        fireStations: stationData,
        edges: puneEdges,
      });

      const dispatchResult = {
        station: apiResult.station,
        reason: apiResult.reason,
        allStations: apiResult.allStations,
        recommendedResources: apiResult.recommendedResources,
        priorityScore: apiResult.priorityScore,
      };
      if (!dispatchResult || !dispatchResult.station) {
        setStep(0);
        setDispatchError('C++ dispatch server returned no reachable fire station.');
        return;
      }
      ...
      const resources = dispatchResult.recommendedResources || computeFireResources(intensity, buildingType);

      const resultData = {
        priorityScore: dispatchResult.priorityScore ?? Math.min(10, intensity + 1),
      ...
      fetch('http://localhost:3001/api/db/incidents', {
        method: 'POST',
      ...
      console.error('[NEXUS] C++ fire dispatch failed:', error.message);
      setStep(0);
  ...
  const pipelineSteps = [
    ...
    { label: '0/1 Knapsack Dynamic Programming', done: step >= 4 },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6" style={{ minHeight: 'calc(100vh - 200px)' }}>
          ...
          <div className="flex flex-col gap-4 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 150px)' }}>
            <GlassPanel variant="warning">
              ...
              <div className="mb-3 p-3 rounded-lg bg-[#ffb800]/5 border border-[#ffb800]/10">
                ...
                <div className="text-[11px] font-mono text-[rgba(255,255,255,0.72)] leading-5">
                  ...
                  <br />
                  Priority queue ranks fire urgency
                  <br />
                  Dijkstra computes shortest station-to-fire path
                  <br />
                  0/1 Knapsack selects the best station under resource budget
                </div>
              </div>

              {dispatchError && (
                <div className="mb-3 p-3 rounded-lg bg-[#ff2d55]/10 border border-[#ff2d55]/20 text-[11px] font-mono text-[#ff9ab0]">
                  {dispatchError}
                </div>
              )}
