...
import { puneNodes, incidentNodes, fireStationNodes } from '../data/puneNodes';
...
const FirePage = () => {
  ...
  const handleDispatch = async () => {
    ...
    try {
      ...
      const trucksNeeded = Math.min(Math.ceil(intensity / 3), 4);
      const firefightersNeeded = intensity * 3 + 5;

      const dispatchResult = {
        priorityScore: Math.min(10, intensity + 1),
        eta: `${Math.max(3, Math.round(pathResult.total_distance * 1.8))} min`,
        totalDistance: pathResult.total_distance,
        route: routeLabels,
        spreadRadius: `${spreadRadius}m`,
        resourcesAllocated: {
          trucks: trucksNeeded,
          firefighters: firefightersNeeded,
          waterTanks: Math.ceil(trucksNeeded * 0.8),
        },
      };
      ...
      await supabase.from('incidents').insert({
        ...
        algorithm_used: 'Knapsack + Dijkstra',
        status: 'dispatched',
      });
    } catch {
      ...
      const dispatchResult = {
        priorityScore: Math.min(10, intensity + 1),
        eta: `${Math.max(3, Math.round(totalDistance * 1.8))} min`,
        totalDistance,
        route: routeLabels,
        spreadRadius: `${spreadRadius}m`,
        resourcesAllocated: {
          trucks: trucksNeeded,
          firefighters: intensity * 3 + 5,
          waterTanks: Math.ceil(trucksNeeded * 0.8),
        },
      };
      ...
      await supabase.from('incidents').insert({
        type: 'fire', location_name: puneNodes.find(n => n.id === selectedOrigin)?.label, map_node_id: selectedOrigin,
        severity: intensity, priority_score: Math.min(10, intensity + 1), route: path, route_distance_km: totalDistance,
        eta: dispatchResult.eta, resources_allocated: dispatchResult.resourcesAllocated,
        algorithm_used: 'Knapsack + Dijkstra (client)', status: 'dispatched',
      }).catch(() => {});
    }
    setLoading(false);
  };
            ...
            <GlassPanel variant="warning">
              ...
              <div className="mb-3">
                ...
                <select value={selectedStation || ''} onChange={(e) => setSelectedStation(e.target.value)}
                  className="input-field" style={{ borderColor: 'rgba(255,184,0,0.3)' }}>
                  <option value="" className="bg-[#0d0d15]">Click station or select...</option>
                  {fireStationNodes.map(n => <option key={n.id} value={n.id} className="bg-[#0d0d15]">{n.label}</option>)}
                </select>
              </div>
