...
const SimulationPage = () => {
  ...
  const handleFullSimulation = async () => {
    ...
    const hospitals = puneNodes.filter(n => n.type === 'hospital');
    const policeStations = puneNodes.filter(n => n.type === 'police_station');
    const fireStations = puneNodes.filter(n => n.type === 'fire_station');
    const randomHospital = () => hospitals[Math.floor(Math.random() * hospitals.length)].id;
    const randomPS = () => policeStations[Math.floor(Math.random() * policeStations.length)].id;
    const randomFS = () => fireStations[Math.floor(Math.random() * fireStations.length)].id;
    ...
    // Legacy Supabase storage is disabled; this is kept as a placeholder only.
